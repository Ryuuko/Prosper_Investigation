...
ggplot(aes(x = LoanOriginalAmount, y = Investors), data = simplified_pLD) + geom_point(alpha = 0.05,
             position = position_jitter(w = 500, h = 20)) + scale_x_continuous(limits = c(0, 40000), breaks = seq(0, 40000, 2500)) + geom_smooth()  
```
...
library(ggplot2)
ggplot(aes(x = LoanOriginalAmount, y = Investors, color = ProsperRating..Alpha.), data = subset(simplified_pLD, ProsperRating..Alpha.!= "")) + geom_point(
             position = position_jitter(w = 500, h = 20)) + scale_color_brewer(type = 'seq', guide = guide_legend(title = "Prosper Rating", override.aes = list(alpha = 1, size = 2), reverse=T)) + theme_dark()
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(aes(x = LoanOriginalAmount, y = Investors), data = subset(simplified_pLD, ProsperRating..Alpha.!= "")) + facet_wrap( ~ ProsperRating..Alpha.) + geom_point()
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#using the full dataset since the prosper score is factorized. It will be better to present the data in continuous legend, even we know the score is discrete, because it will difficult to read if we turn them into 11 different colors

ggplot(aes(x = LoanOriginalAmount, y = Investors, color = ProsperScore), data = subset(pLD, ProsperRating..Alpha.!= "")) + geom_point(position = position_jitter(w = 500, h = 20)) + scale_colour_gradient(low = "white", high="blue") + theme_dark()
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(aes(x = LoanOriginalAmount, y = Investors, color = LenderYield), data = subset(simplified_pLD, ProsperRating..Alpha.!= "")) + geom_point(position = position_jitter(w = 500, h = 20)) + scale_colour_gradient(low = "white", high="red")
```
...
ggplot(aes(x = LoanOriginalAmount, y = Investors, color = LenderYield), data = subset(simplified_pLD, ProsperRating..Alpha.!= "")) + geom_point(position = position_jitter(w = 500, h = 20)) + scale_colour_gradient(low = "white", high="red") + facet_wrap(~ ProsperRating..Alpha.)
...
ggplot(aes(x = LenderYield, y = Investors), 
              data = subset(simplified_pLD, ProsperRating..Alpha.!= "")) + geom_line(aes(color = ProsperRating..Alpha.), stat = 'summary', fun.y = mean) + facet_wrap(~ ProsperRating..Alpha.)
```
...
The differences between different grades become so obvious. From HR-grade to C-grade, investors are going to invest more and more. From C-grade to AA-grade, the amount of individual investment start to shrink.
...
ggplot(aes(x = LenderYield, y = LoanOriginalAmount/Investors, group=Term), 
              data = subset(simplified_pLD, ProsperRating..Alpha.!= "")) + geom_line(aes(linetype=Term, color=Term), stat = 'summary', fun.y = mean) + scale_linetype_manual(values=c("dotted", "dashed",  "solid")) + facet_wrap(~ ProsperRating..Alpha., scales = "free") + theme_light()
```

Why this time the A-grade borrower can attract more investors to invest more in amount than the C-grade borrower? I think the reason is that the last graph is an average result. The lines in A-grade don't consistently hit the peak of invesotrs.

In general, C-grade and B-grade can attract investors to invest in larger amount across different rates of yield in 36-term and 60-term, they just don't hit the peak of the numbers when their number is split into three categories.

```{r echo=FALSE, message=FALSE, warning=FALSE}
# in order to keep identical to the color before, not going to tune into the sequential color

ggplot(aes(x = LenderYield, y = LoanOriginalAmount/Investors), 
              data = subset(simplified_pLD, ProsperRating..Alpha.!= "")) + geom_density(aes(fill = ProsperRating..Alpha., color = ProsperRating..Alpha.), stat = 'summary', fun.y = mean) + facet_wrap(~ Term) + theme_light() + scale_color_brewer(type = 'seq') + scale_fill_brewer(type = 'seq')

```

Some of the people may want to invest in borrowers with A-grade with huge amount of money if they are going to invest in a long term in 5-year-loan. I think it is because people want to balance the need of safe investment and more return in a long-term investment. But the density of A-grade lines are actually similar to the density of B-grade, showing the peak may be just the minority.

In the term of 36, The density of A-grade line are so similar to lines from B-grade C-grade.
...
ggplot(aes(Investors), data = pLD) + geom_histogram() + scale_x_log10(
   breaks = scales::trans_breaks("log10", function(x) 10^x),
   labels = scales::trans_format("log10", scales::math_format(10^.x))
 ) + labs(y = "Number of Loans", x = "Number of Investors", title = "Log_10 Investors Number")
```
...
ggplot(aes(x = ProsperRating..Alpha., y = Investors), data = subset(simplified_pLD, ProsperRating..Alpha. != "")) +
        geom_boxplot(fill = fill, colour = line) + coord_cartesian(ylim=c(0, 300)) + labs(y = "Number of Investors", x = "Prosper Rating (in Alphabet)", title = "Number of Investors by Prosper Rating (in Alphabet)")
...
ggplot(aes(x = LenderYield, y = LoanOriginalAmount/Investors), 
              data = subset(simplified_pLD, ProsperRating..Alpha.!= "")) + geom_density(aes(fill = ProsperRating..Alpha., color = ProsperRating..Alpha.), stat = 'summary', fun.y = mean) + facet_wrap(~ Term) + scale_color_brewer(type = 'seq') + scale_fill_brewer(type = 'seq') + labs(y = "Average investing amount of an investor (in dollar) ", x = "Rate of Lender Yield", title = "Average Investing amount of an investor by Lender Yield, Prosper Rating and Terms", color = "Prosper Rating (in Alphabet)", fill = "Prosper Rating (in Alphabet)") + theme_light()
...
I may put too much time on reviewing variables in uni-variable analyze and lack of thoughts about the meaning of number of investors. I hope at the end, everything looks good.
...
Also, I hope one day I can find the mystery of the credit score as, if I have time to do so. This project really takes time and it is not as easy as I thought before. 